...
%W                                                    & Burkhard Hoefling
%%
%H  @(#)$Id: install.msk,v 1.123.2.6 2004/03/17 20:44:31 gap Exp $
%%
...
\beginitems
Unix, OS X&
Under Unix or OS X unpack the archive `gap4r4p2' in whatever
...
`unzoo 4.4 PPC'. 
Now move this applications, as well as the `gap4r4p2.zoo'
archive to the Folder in which you want to install {\GAP}. Drag the archive
`gap4r4p2.zoo onto the icon of `unzoo 4.4 PPC'. You will get a
...
archives we provide move `unzoo.exe' and  `gap4r4p2.zoo' in the
...
"/usr/local/lib/gap4r4/tst/testall.g"
gap> Read(last);
\endtt

The test requires about 60-70MB of memory and runs about 2 minutes on a
PentiumIII/1GHz machine.  You will get a large number of lines with output
about the progress of the tests.
\begintt
...
To extract the packages, extract the `package4r4p2' archive *in*
...
get the `htmie4r4p2' archive which provides math symbols in a
different encoding. (`Mozilla', `Konqueror' and `Safari' all support unicode
characters.)

The manual is also available in `pdf' format.
In the full UNIX
distribution (`gap4r4p2.zoo') these files are included in the
directory
`gap4r4/doc'  in the
...
Also \URL{http://www.gap-system.org/Info4/windows.html} might give
a remedy.
...
folder you want to install {\GAP} 4.4.2.
...
distribution   `gap4r4p2.zoo' and the    binary    archive
`unzoo4r4-PPC.sit',
...
Move the file `gap4r4p2.zoo' into the folder `Applications'
...
(You will not need the file `gap4r4p2.zoo' any
