...
% DO NOT EDIT!
                  README for GAP 4.4.5

These archives contain the current release of GAP. It is version 4.4.5
...
For a complete installation fetch the files
   ...
   gap4r4p5    ....   The main GAP system
   packages4r4p5  ....   Contributed packages
...
While the `gap4r4p5' archive must be unpacked *in* the directory
in which you want GAP's root directory to sit (say, '/usr/local/lib/', a
subdirectory 'gap4r4' is created), the
`packages4r4p5' archive must be extracted into the
...
Decompress  'gap4r4p5-win.zip'   on  the  top-level  directory   of  drive  C:\.
This  creates  a  directory  C:\GAP4R4,   the  'GAP  root  directory'.  Then
extract  'packages4r4p5-win.zip' into  C:\GAP4R4\pkg\.   
Click  on C:\gap4r4p5\bin\gapw95.exe to start GAP.
...
Move this application and gap4r4p5.zoo  into the  folder in
which you want  to install  GAP and decompress  it by dragging it onto the
'unzoo' application icon.

.  This  should  have  created  a  folder  'gap4r4p5' with a 
'GAP 4 ...' application in this directory.

To install the GAP packages,  move 'packages4r4p5.zoo' into the  
