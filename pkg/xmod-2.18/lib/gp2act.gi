...
function( XM )
    ...
    p2aS := GroupHomomorphismByImagesTriv( PAS, AS, genPAS, genAS );
    ...
    p2aR := GroupHomomorphismByImagesTriv( PAR, AR, genPAR, genAR );
    ...
    emAS := GroupHomomorphismByImagesTriv( AS, D, genAS, emgenPAS );  
    emAR := GroupHomomorphismByImagesTriv( AR, D, genAR, emgenPAR ); 
    ...
    projS := GroupHomomorphismByImagesTriv( P, PAS, genP, imsrc );
    projR := GroupHomomorphismByImagesTriv( P, PAR, genP, imrng );
    ### 21/06/06 ### genPAS := GeneratorsOfGroup( PAS );
    ePAS := GroupHomomorphismByImagesTriv( PAS, D, genPAS, genPAS );
    ### 21/06/06 ### genPAR := GeneratorsOfGroup( PAR );
    egenR := List( genPAR, p -> p^p2 );
    ePAR := GroupHomomorphismByImagesTriv( PAR, D, genPAR, egenR );
...
function( XM )
    ...
    p2aS := GroupHomomorphismByImagesTriv( PAS, AS, genPAS, genAS );
    SetAutoGroupIsomorphism( PAS, p2aS );
    restrict := GroupHomomorphismByImagesTriv( PAR, PAS, genPAR, genPAS );
    ...
    emAS := GroupHomomorphismByImagesTriv( AS,D,genAS{filtS},emgenPAS{filtS} );
    emAR := GroupHomomorphismByImagesTriv( AR,D,genAR,       emgenPAR );
    infoD := DirectProductInfo( D );
    ...
    projS := GroupHomomorphismByImagesTriv( P, PAS, genP, imsrc );
    projR := GroupHomomorphismByImagesTriv( P, PAR, genP, imrng ); 
    ePAS := GroupHomomorphismByImagesTriv( PAS,D,genPAS{filtS},genPAS{filtS} );
    egenR := List( genPAR, p -> p^p2 );
    ePAR := GroupHomomorphismByImagesTriv( PAR,D,genPAR,       egenR ); 
    SetGeneratingAutomorphisms( XM, autogens );
    SetIsAutomorphismPermGroupOfXMod( P, true );
...
function( mor, chi )
    ...
    invrho := GroupHomomorphismByImagesTriv( R, R, imrho, stgR );
    imj := 0 * rngR;
    for k in rngR do
        r := stgR[k];
        rr := Image( invrho, r );
...
function( XM )
    ...
    od;
    iota := GroupHomomorphismByImagesTriv( S, W, genS, imiota );
    ...
    if ( genchi = [ ] ) then
        ...
        for j in [1..nposW] do
            ...
            a := GroupHomomorphismByImagesTriv( S, S, genS, ima );
            imact[j] := a;
        od;
    fi; 
    act := GroupHomomorphismByImagesTriv( W, autS, genW, imact );
    WX := XMod( iota, act );
    name := Name( XM );
...
Info( InfoXMod, 2, "now in NorrieXMod" ); 
    ...
    od;
    bdy := GroupHomomorphismByImagesTriv( R, P, genR, im );
    ...
    for i in Prng do
        ...
        a := GroupHomomorphismByImagesTriv( R, R, genR, ima );
        imact[i] := a;
    od;
    act := GroupHomomorphismByImagesTriv( P, AR, genP, imact );
...
function( XM )
    ...
    for i in Prng  do
        ...
        imact[i] := a;
    od;
    act := GroupHomomorphismByImagesTriv( P, AS, genP, imact );
    for f in MappingGeneratorsImages( act )[2] do
...
function( XM )
    ...
    od;
    delta := GroupHomomorphismByImagesTriv( W, P, genW, imdelta );
    ...
    for k in [1..nGA] do
        ...
        phi := GroupHomomorphismByImagesTriv( W, W, genW, imgen );
        imact[k] := phi;
    od;
    id := InclusionMappingGroups( W, W );
    aut := Group( imact, id );
    SetName( aut, "Aut(W)" );
    act := GroupHomomorphismByImagesTriv( P, aut, genP, imact );
    ActX := XMod( delta, act );
    name := Name( XM );
