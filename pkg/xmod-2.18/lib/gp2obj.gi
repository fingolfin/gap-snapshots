...
function( bdy, act )
    ...
    for i in [ 1..Length( imact ) ] do
        ...
        a := GroupHomomorphismByImagesTriv( src, src, gensrc, ima );
        imact[i] := a;
    od;
...
function( XM )
    ...
    if IsTrivialAction2dDomain( XM ) then
        ...
        t := GroupHomomorphismByImagesTriv( G, Xrng, genG, imt );
        h := GroupHomomorphismByImagesTriv( G, Xrng, genG, imh );
        eR := Embedding( G, 1 );
...
function( G, N )
    ...
    bdy := GroupHomomorphismByImagesTriv( N, G, gensrc, gensrc );
    autgen := [ ];
    for g in genrng do
        imautgen := List( gensrc, n -> n^g );
        a := GroupHomomorphismByImagesTriv( N, N, gensrc, imautgen );
        Add( autgen, a );
    od;
    ...
    act := GroupHomomorphismByImagesTriv( G, aut, genrng, autgen );
...
function( hom )
    ...
    for j in [1..ngsrc] do 
        ...
        autgen[j] := GroupHomomorphismByImagesTriv( src, src, gensrc, imsrc );
    od;
    aut := Group( autgen );
    SetIsGroupOfAutomorphisms( aut, true );
    Info( InfoXMod, 2, "Group of conjugations has size ", Size(aut) );
    act := GroupHomomorphismByImagesTriv( rng, aut, imhom, autgen );
    if ( not IsGroupHomomorphism( act ) ) then
...
function( G, A )
    ...
    p2a := GroupHomomorphismByImagesTriv( P, A, genP, genA );
    abelian := IsAbelian( G );
    genG := GeneratorsOfGroup( G );
    oneP := One( P );
    if abelian then
        ...
        for g in genG do
            ima := List( genG, h -> h^g );
            a := GroupHomomorphismByImagesTriv( G, G, genG, ima );
            img := Image( a2p, a );
            Add( imbdy, img );
        od;
    fi;  
    bdy := GroupHomomorphismByImagesTriv( G, P, genG, imbdy );
...
function( PM )
    ...
    bdy := GroupHomomorphismByImagesTriv( quot, PMrng, qgen, impqgen );
    ...
    for r in genrng do
        ...
        phi := GroupHomomorphismByImagesTriv( quot, quot, qgen, autgen );
        Add( imautgen, phi );
    od;
    aut := Group( imautgen );
    act := GroupHomomorphismByImagesTriv( PMrng, aut, genrng, imautgen );
...
function( PM, Ssrc, Srng )
    ...
    if ( trivrng or trivsrc ) then
        ...
        Sbdy:= GroupHomomorphismByImagesTriv( Ssrc, Srng, genSsrc, imgen );
    fi;
    innaut := [ ];
    for r in genSrng do
        alpha := Image( Pact, r );
        ...
        alpha:=GroupHomomorphismByImagesTriv( Ssrc, Ssrc, genSsrc, imgen );
        Add( innaut, alpha );
    od;
    ...
    Sact := GroupHomomorphismByImagesTriv( Srng, Saut, genSrng, innaut );
...
function( size, gpnum, num )
    ...
    local  ok, type, norm, usage, usage2, maxsize,
           start, iso, count, pos, pos2, names,
           i, j, ncat1, G, genG, M, L, genR, R, t, kert, h, C1G, XC;

    maxsize := CAT1_LIST_MAX_SIZE;
    usage := "Usage:  Cat1( size, gpnum, num );";
    ...
    if ( num = 1 ) then
        L := [ genG, Name(G), Name(G), genG, genG ];
    else
        L := M[5][num-1];
    fi;
    genR := L[1];
    R := Subgroup( G, genR );
    SetName( R, L[3] );
    t := GroupHomomorphismByImagesTriv( G, R, genG, L[4] );
    h := GroupHomomorphismByImagesTriv( G, R, genG, L[5] );
    SetName( Image( t ), L[3] );
    SetName( Image( h ), L[3] );
...
function( t, h, e )
    ...
    hres := GroupHomomorphismByImagesTriv( G, R, genG, imh );
    tres := GroupHomomorphismByImagesTriv( G, R, genG, imt );
    eres := GroupHomomorphismByImagesTriv( R, G, genR, ime );
    kergen := GeneratorsOfGroup( kert );
    imbdy := List( kergen, x -> Image( h, x) );
    bdy := GroupHomomorphismByImagesTriv( kert, R, kergen, imbdy );
...
function( et, eh )
    ...
    t := GroupHomomorphismByImagesTriv( G, R, gG, List( gG, g->Image(et,g) ) );
    h := GroupHomomorphismByImagesTriv( G, R, gG, List( gG, g->Image(eh,g) ) );
...
function( C1G )
    ...
    if IsIdentityCat1( C1G ) then
      ...
      # aut.identity.genimages := genker;
        ...
        for r in genrng do
            ...
            a := GroupHomomorphismByImagesTriv( kert, kert, genker, imautgen );
            Add( autgen, a );
        od;
        idkert := IdentityMapping( kert );
        aut := Group( autgen, idkert );
        act := GroupHomomorphismByImagesTriv( Crng, aut, genrng, autgen );
    ...
    if ( HasName( kert ) and HasName( Crng ) ) then 
        name := Name( PM ); 
...
function( PC )
    ...
    tail := GroupHomomorphismByImagesTriv( quot, PCrng, qgen, tpqgen );
    hpqgen := List( pqgen, p -> Image( PCh, p ) );
    head := GroupHomomorphismByImagesTriv( quot, PCrng, qgen, hpqgen );
    ime := List( genrng, r -> Image( nat, Image( PCe, r ) ) );
    embed := GroupHomomorphismByImagesTriv( PCrng, quot, genrng, ime );
...
function( list, X1 )
    ...
    bdy := GroupHomomorphismByImagesTriv( S, R, genS, imbdy );
    autgen := 0 * [ 1..lenR ];
    for i in XRpos do
        ...
        od;
        alpha := GroupHomomorphismByImagesTriv( S, S, genS, imaut );
        autgen[i] := alpha;
    od; 
    if spec[2] then 
        k := 0; 
    ...
    od;
    aut := Group( autgen );
    act := GroupHomomorphismByImagesTriv( R, aut, genR, autgen );
...
##############################################################################
##
#M  IdentitySub2dDomain . . . . . . . . . . . . . . . of a 2d-object
...
##
InstallMethod( IdentitySub2dDomain, "of a 2d-object", true, [ Is2dDomain ], 0,
function( obj )
...
InstallMethod( IdentitySubPreXMod, "of a pre-crossed module", true,
    [ IsPreXMod ], 0,
function( obj )
    return IdentitySub2dDomain( obj );
end );
...
function( obj )
    ...
    if IsXMod( obj ) then
        ...
        return IsNormalSubgroup2dDomain( XModByCat1( obj ) );
    else
