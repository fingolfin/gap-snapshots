    ...
    if ( P2S = fail ) then
        ...
        iso := G2P;            
            ...
            for j in [1..numG] do
                ...
                imiso[j] := Image( P2S, p );
            od; 
            iso := GroupHomomorphismByImagesTriv( G, S, genG, imiso );
        fi;
    fi;
...
function( D, i )
    ...
    hom := GroupHomomorphismByImagesTriv( G, D, genG, imgs );
...
function( D )
    ...
    hom := GroupHomomorphismByImagesTriv( D, G, genD, imgs );
    ...
    for i in [1..clnum] do
        ...
        for k in [1..cjnum] do
            ...
            for l in [1..Lnum] do
                im := LR[l];
                gim := List( im, x -> x^g );
                psi2 := GroupHomomorphismByImagesTriv( G, G, genG, gim );
...
function( grp )
    ...
    mgi := MappingGeneratorsImages( iso );
    inv := GroupHomomorphismByImagesTriv( perm, grp, mgi[2], mgi[1] );
...
function( grp )
    ...
    pc := ImagesSource( iso );
    mgi := MappingGeneratorsImages( iso );
    inv := GroupHomomorphismByImagesTriv( pc, grp, mgi[2], mgi[1] );
    if IsFpGroup( grp ) then
