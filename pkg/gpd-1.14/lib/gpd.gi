...
SUB_CONSTRUCTORS := Concatenation( 
    ...
    "5.  FullIdentitySubgroupoid( groupoid );\n", 
    "6.  DiscreteIdentitySubgroupoid( groupoid );\n", 
...
## temp!    gens := GeneratorsOfMagmaWithObjects( gpd ); 
    return gpd; 
end );
...
function( G, gps, obs )
    ...
    len := Length( obs ); 
    if ( len = 1 ) then 
        Print( "#I since len=1 should call DomainWithSingleObject ?\n" ); 
    fi; 
...
##
#M  FullSubgroupoid
#M  FullIdentitySubgroupoid
#M  DiscreteIdentitySubgroupoid
##
...
InstallMethod( FullIdentitySubgroupoid, "for a connected groupoid", 
    true, [ IsGroupoid and IsSinglePiece ], 0,
function( gpd ) 
    local  id, sub;
    id := IdentitySubgroup( gpd!.magma );
    sub := SinglePieceGroupoidNC( id, gpd!.objects );
    SetParentAttr( sub, gpd );
    return sub;
end );

InstallMethod( FullIdentitySubgroupoid, "generic method for a groupoid", 
    true, [ IsGroupoid ], 0,
function( gpd )
    local  pieces, subs;
    pieces := Pieces( gpd );
    subs := List( pieces, c -> FullIdentitySubgroupoid(c) );
    return UnionOfPieces( subs );
end );

InstallMethod( DiscreteIdentitySubgroupoid, "for a connected groupoid", 
    true, [ IsGroupoid and IsSinglePiece ], 0,
function( gpd )
    local  id, obs, gps;
    obs := gpd!.objects;
...
InstallMethod( DiscreteIdentitySubgroupoid, "generic method for a groupoid", 
    true, [ IsGroupoid ], 0,
function( gpd )
    local  pieces, subs;
    pieces := Pieces( gpd );
    subs := List( pieces, c -> DiscreteIdentitySubgroupoid(c) );
    return UnionOfPieces( subs );
end );
