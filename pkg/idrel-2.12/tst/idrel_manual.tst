...
##
#Y  Copyright (C) 1999--2012 Anne Heyworth and Chris Wensley
##
...
gap> for j in [1..8] do
Syntax error: od expected in stream line 2
^
gap>    r := r0[j];;
Error, Variable: 'j' must have a value
gap>    if (j<5) then
Error, Variable: 'j' must have a value
Syntax error: fi expected in stream line 2
^
gap>       l0[j] := [ r[1], [ [j,id] ], r[2] ];;
Error, Variable: 'j' must have a value
gap>    else
...
gap> Length( l1 );
Error, Variable: 'l1' must have a value
...
Error, Variable: 'l2' must have a value
...
gap> Print( pg * pr, "\n" );
28*f2^5 - 35*f2*f1*f2*f1*f2^-1 - 14*f2*f1^2*f2^2 + 21*f2*f1^4 + 24*f1*f2^4 - 
...
Syntax error: expression expected in stream line 2
^
...
Error, Variable: 's2' must have a value
gap> One( s1 );
<identity ...>
gap> Terms( s1 );
