...
##
#H   @(#)$Id: triangulations.g,v 1.40 2012/06/03 14:48:37 gap Exp $
##
...
BindGlobal("ADDTOTRIANGULATION@", function(t,p)
    ...
    f := LOCATE@(t,fail,p);
    while Length(f)=4 do # vertex
        Error("Two vertices coincide: ",p," and ",f[4]);
    od;
    f := f[1];
...
BindGlobal("ISVERTEX@", r->IsBound(r.n) and IsBound(r.pos));
BindGlobal("ISEDGE@", r->IsBound(r.to));
BindGlobal("ISFACE@", r->IsBound(r.n) and not IsBound(r.pos));
...
BindGlobal("NORMALIZINGMAP@", function(points,oldpoints,fast)
...
BindGlobal("FRMACHINE2RAT@", function(z,M)
    ...
    repeat
        ...
        mobius := NORMALIZINGMAP@(sublifts,VERTICES@(spider),fast);
