...
#T is this really still necessary?
    ...
    result:= 1;
    for i in [ 1 .. CTblLib.MAXPRIMRANGE ] do
      result:= result * i;
...
#T if the factorial value is not cached then we can do better
...
##
##  The individual entries are added via 'NotifyGroupInfoForCharacterTable'.
##
...
##
CTblLib.Data.IdEnumeratorExt:= DatabaseIdEnumerator( rec(
    identifiers:= [],
    isSorted:= false,
    entry:= function( idenum, id ) return CharacterTable( id ); end,
    version:= "",
...
CallFuncList( function()
    ...
    for name in Difference( RecNames( CTblLib.Data.IdEnumerator.attributes ),
      ...
      DatabaseAttributeAdd( CTblLib.Data.IdEnumeratorExt, r );
    od;
    end, [] );
