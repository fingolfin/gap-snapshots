...
##
##  Create the data list from the LaTeX file 'app2a.tex' (if you have it).
##
CTblLib.AtlasErrorsData:= function( filename )
    local str, sections, records, sstr, pos, pos2, r, new, start,
          next, curr, line, type, text, page, lpage, lgroup, group, entry;

    # Fetch the sections from ``Introduction'' to ``Bibliography''.
...
#############################################################################
##
#F  BrowseAtlasImprovements()
##
##  <#GAPDoc Label="BrowseAtlasImprovements">
##  <ManSection>
##  <Func Name="BrowseAtlasImprovements" Arg=''/>
##
##  <Returns>
##  nothing.
##  </Returns>
...
##    and <C>2</C> for entries from <Cite Key="AtlasImpII"/>,
##  </Item>
##  <Mark><C>Typ</C></Mark>
##  <Item>
...
##  <Mark><C>Page</C></Mark>
##  <Item>
##    the page and perhaps the line in the &ATLAS;,
##  </Item>
##  <Mark><C>Group</C></Mark>
##  <Item>
...
##    to which some statements in <Cite Key="BMO17"/> refer
##    (one of <C>I</C>, <C>CH</C>, <C>G</C>, <C>MX</C>, <C>FI</C>, <C>P</C>,
##    <C>M</C>),
##  </Item>
##  </List>
##  <P/>
...
##  <#/GAPDoc>
##
BindGlobal( "BrowseAtlasImprovements", function()
    local file, str, simplifiedPageNumber, simplifiedGroupName,
          formattedText, matrix, sections, r, entry, row, pagewidth,
          groupwidth, descrwidth, t, i;

    if not IsBound( CTblLib.AtlasImprovements ) then
    ...
    matrix:= [];
    sections:= [];
    for r in CTblLib.AtlasImprovements do
      if not r.section in sections then
        Add( sections, r.section );
      fi;
      for entry in r.entries do
        if Length( entry ) = 4 then
          row:= Concatenation( [ r.section, r.source ], entry, [ "" ] );
        else
          row:= Concatenation( [ r.section, r.source ], entry );
        fi;
