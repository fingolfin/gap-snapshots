...
function( mag1, mag2, images )
    ...
    local  fam, filter, map, ok, imo, homs;
    ...
    #?  made a mess here (07/11/08 ??
    #?  have another look at the categories involved ??
    if ( IsGroupoid( mag1 ) and IsGroupoid( mag2 ) ) then 
        ## ?? (29/04/10) SetIsGroupoidHomomorphism( map, true ); 
        Info( InfoGpd, 2, "should not be using HomomorphismToSinglePiece" ); 
    elif ( IsMagmaWithObjectsAndOnes( mag1 )   
...
#M  ViewObj
##
InstallMethod( PrintObj, "for a magma with objects homomorphism", true,
    [ IsMagmaWithObjectsHomomorphism ], 0, ViewObj ); 

InstallMethod( ViewObj, "for a mwo homomorphism to single piece", true,
    [ IsHomomorphismToSinglePiece ], 0, 
function ( hom ) 
...
InstallMethod( ViewObj, "for a general mwo homomorphism", true,
    [ IsMagmaWithObjectsHomomorphism ], 0, 
function ( hom ) 
    Print( "magma with objects homomorphism : " );  
    if ( HasName( Source(hom) ) and HasName( Range(hom) ) ) then 
...
InstallMethod( ViewObj, "for a groupoid homomorphism", true,
...
InstallMethod( ViewObj, "for a mapping from homogeneous discrete gpd", true,
    [ IsGroupoidHomomorphism and IsGeneralMappingFromHomogeneousDiscrete ], 0, 
function ( hom ) 
    local  h; 
    if ( HasName( Source(hom) ) and HasName( Range(hom) ) ) then 
        Print( Source(hom), " -> ", Range(hom), "\n" ); 
...
#? this gets called - but it shouldn't - when src,rng are just magmas ?? 
InstallOtherMethod( ImageElm, "for a semigroup mapping", true,
  [ IsMagmaWithObjectsHomomorphism, IsMultiplicativeElementWithObjects ],
  0,
function ( map, e )
   Info( InfoGpd, 3, "this is the second ImageElm function in mwohom.gi" ); 
    ## Print( "not yet implemented ImageElm for a general mapping\n" );
    TryNextMethod(); 
end );
