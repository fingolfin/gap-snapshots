...
function( v, e )
    ...
    od;
    return FpWeightedDigraphNC( v, e );
end);
...
function( dig )
    if HasName( dig ) then
        ...
        Print("weighted digraph with vertices: ", dig!.vertices, "\n");
        Print("and arcs: ", dig!.arcs, "\n" );
        ## Print("weighted digraph with vertices: ", Vertices( dig ), "\n");
        ## Print("and arcs: ", Arcs( dig ), "\n" );
    fi;
end );
...
function( dig, gps, subgps, isos )
    ...
    SetSubgroupsOfGraphOfGroups( gg, subgps );
...
function( dig, gps, subgps, isos )
    ...
    if ((lenV <> Length(gps)) or (lenE <> Length(subgps)) or 
           (lenE <> Length(isos))) then
    ...
    od;
    # checking that subgroups are groups
    for i in [1..lenE] do
        if ( IsGroup( subgps[i] )=false ) then
            Error( "not a subgroup");
        fi;
    od;
    # checking that subgroups are subgroups of the vertex group
    for i in [1..lenE] do
        pos := Position(v, e[i][2]); 
        if ( IsSubgroup( gps[pos], subgps[i] )=false ) then
            Error( "subgroups are not of correct groups" );
        fi;
    od;
...
###        einvpos := Position( e, e[inv[i]] );
        ...
        od;
    od;
    return GraphOfGroupsNC( dig, gps, subgps, isos );
end );
...
return ( ( DigraphOfGraphOfGroups(gg1) = DigraphOfGraphOfGroups(gg2) )
     ...
     and ( SubgroupsOfGraphOfGroups(gg1) = SubgroupsOfGraphOfGroups(gg2) )
...
function( gg )
    ...
    Print( "        arcs: ", dig!.arcs, "\n" );
    Print( "      groups: ", GroupsOfGraphOfGroups( gg ), "\n" );
    Print( "   subgroups: ", SubgroupsOfGraphOfGroups( gg ), "\n" );
...
function( gg )
    ...
    local  gps, dig, subgps, adig, vdig, len, i, g, rc, rep, trans;

    gps := GroupsOfGraphOfGroups( gg );
    dig := DigraphOfGraphOfGroups( gg );
    subgps := SubgroupsOfGraphOfGroups( gg );
    vdig := dig!.vertices;
    adig := dig!.arcs;
    ## vdig := Vertices( dig );
    ## adig := Arcs( dig );
...
function( ggword )
    ...
    ## adig := Arcs( DigraphOfGraphOfGroups( gg ) );
...
function( ggword )
    ...
    ## return Arcs( DigraphOfGraphOfGroups( gg ) )[e][3];
end );
...
function( ggword )
    ...
    local  w, tw, hw, gg, gps, sgps, dig, adig, vdig, lw, len, k, k2, he, 
           rtrans, tran, pos, a, g, h, found, i, nwit, tsp,
           isos, im, sub, u, v, gu, gv, e, ie, ng, rw, lenred, isfp, isid;
    ...
    gg := GraphOfGroupsOfWord( ggword );
    gps := GroupsOfGraphOfGroups( gg );
    sgps := SubgroupsOfGraphOfGroups( gg );
...
function ( ggw, n )
    ...
    if ( n = 0 ) then
        ...
        ## ptv := Position( Vertices( DigraphOfGraphOfGroups( gg ) ), tv );
...
function( fpa )
    ...
    verts := [5,6];
    arcs := [ [y,5,6], [y^-1,6,5]];
    dig := FpWeightedDigraph( verts, arcs );
...
#    e2 := info!.embeddings[2];
    ...
    gg := GraphOfGroups( dig, [f1,f2], [h1,h2], [iso,inv] );
    return gg;
end );
...
#    e2 := info!.embeddings[2];
    ...
    h2 := Range( iso );
    inv := InverseGeneralMapping( iso );
    gg := GraphOfGroups( dig, [fp], [h1,h2], [iso,inv] );
    return gg;
end );
