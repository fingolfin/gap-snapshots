...
GROUPOID_MAPPING_CONSTRUCTORS := Concatenation(
    ...
    "5.  HomomorphismToSinglePiece( src, rng, list of [hom,imobs,rays]'s );\n",
    "6.  HomomorphismByUnion( src, rng, list of disjoint mappings );\n", 
    "7.  GroupoidHomomorphism( one of the previous parameter options );" );
...
InstallMethod( Display, "for a mapping of connected groupoids", true,
    [ IsGroupoidHomomorphism and IsHomomorphismToSinglePiece ], 0,
function ( map ) 
...
function( gpd, oims )
    ...
    if ( pos = fail ) then 
        Error( "object images not a permutation of the object" ); 
    fi; 
...
InstallMethod( GroupoidAutomorphismByGroupAuto, 
    "for a groupoid and an automorphism off the root group", true, 
...
function( gpd ) 
    ...
    Info( InfoGpd, 2, "this is the latest attempt at AutomorphismGroup" ); 
    ##  first: automorphisms of the root group 
    autgen := [ ]; 
    rgp := gpd!.magma; 
    genrgp := GeneratorsOfGroup( rgp ); 
...
function( gpd ) 
    ...
    Info( InfoGpd, 2, "attempt at AutomorphismGroup for homogeneous discrete" ); 
    pieces := Pieces( gpd ); 
    obs := ObjectList( gpd ); 
    n := Length( obs );
...
function ( map, e )
    ...
    local  m1, m2, imo, obs1, obs2, pt1, ph1, t2, h2, pt2, ph2, 
           ray1, rims, rh, loop, g2, g2a, iloop;
    ...
    t2 := imo[ pt1 ];
    h2 := imo[ ph1 ]; 
    pt2 := Position( obs2, t2 ); 
    ph2 := Position( obs2, h2 ); 
    rims := ImagesOfRays( map );  
    rh := RootHomomorphism( map ); 
    iloop := ImageElm( rh, loop ); 
    g2 := rims[pt1]^-1 * iloop * rims[ph1]; 
    return MultiplicativeElementWithObjectsNC( true, g2, t2, h2 );
end ); 
...
function ( map, e )
    ...
    local  m1, m2, imo, obs1, obs2, p1, t2, p2, 
           hom, ray1, rims, rh, loop1, g2;
    ...
    obs1 := m1!.objects; 
    obs2 := m2!.objects; 
    p1 := Position( obs1, e![2] ); 
    t2 := imo[ p1 ]; 
    p2 := Position( obs2, t2 ); 
    hom := ObjectHomomorphisms( map )[ p1 ]; 
    g2 := ImageElm( hom, e![1] ); 
...
function ( map, e )
    ...
    Ge := C1[pe];
    obs1 := Ge!.objects;
    hom := PieceImages( map )[pe][2];
    obs2 := PieceImages( map )[pe][1]; 
